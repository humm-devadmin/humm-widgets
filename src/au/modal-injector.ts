...
export class ModalInjector {
    ...
    public replaceBanner(template: string, targetUrl: string, modalId: string, dynamicData: MerchantTerms, element?: JQuery, ) {
        // use this method only when GUID id has been passed 
        if (!this.modalExists(modalId)) {
            this.injectDynamicModal(targetUrl, dynamicData, modalId);
        }
