    ...
    function generateWidget(widgetId): string {
        ...
        if (type == Type.bigThings) {
            ...
            if (productPrice < min || productPrice == 0) {
                little_term = 5;
                main_html = 'with ' + little_term + ' fortnightly payments';
            } else if ((productPrice <= 1000 && productPrice <= max)) {
                little_term = 5;
                main_html = 'with ' + little_term + ' fortnightly payments';
                let fortnightlyPayment = productPrice / little_term;
                // Banking Rounding
                let roundedDownProductPrice = Math.floor(fortnightlyPayment * Math.pow(10, 2)) / Math.pow(10, 2);
                price_breakdown_html = `of <span class="humm-price">$${roundedDownProductPrice.toFixed(2)}</span>`
            } else if ((productPrice > 1000 && productPrice <= 2000 && productPrice <= max)) {
                little_term = 10;
                main_html = 'with ' + little_term + ' fortnightly payments';
                let fortnightlyPayment = productPrice / little_term;
                // Banking Rounding
