...
(($: JQueryStatic) => {
    ...
    function insert(useHTML?: string, replaceElement?: string, dynamicData?: MerchantTerms) {
        let widgetUrl = productPrice <= 2000 ? Config.priceInfoUrl : ( merchantId ? Config.priceInfoAPIModalUrl : Config.priceInfoMoreUrl );
        let widgetId = productPrice <= 2000 ? Config.priceInfoModalId : ( merchantId ? Config.priceInfoAPIModalId : Config.priceInfoMoreModalId );
        if (type == Type.bigThings) {
            if (merchantId) {
